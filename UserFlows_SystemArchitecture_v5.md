...
**Note on Database-Only Indicators:** For privacy, simplicity, and zero external API dependencies, all real-world check-ins, heatmaps, and activity indicators rely entirely on Supabase database queries of user check-ins and RSVPs. No third-party foot traffic APIs or background location services are utilized.
...
2. **Location Permissions:** The app requests foreground geolocation permissions. This is required to populate the local map with active events and check-ins.  
